...
def submit_rating(diamond, rating):
    ...
    save_ratings(st.session_state.ratings_df, output_file)
...
if 'ratings_df' not in st.session_state:
    st.session_state.ratings_df = conn.read(f"{BUCKET_NAME}/{output_file}", input_format="csv", ttl=600)
...
# Option to view current ratings
if st.checkbox("View current ratings"):
    st.write(st.session_state.ratings_df)